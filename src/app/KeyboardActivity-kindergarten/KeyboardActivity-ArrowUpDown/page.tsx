...
const arrowsKeyPressPage = () => {
    ...
    useEffect(() => {
        const handleKeyDown = async (e: KeyboardEvent) => {
            ...
            if (isCorrect = e.code === "ArrowUp" || e.code === "ArrowDown") {
    ...
    return (
        <main className="flex min-h-screen flex-col justify-between bg-[#FAF9F6] font-serif leading-normal tracking-normal text-[#132241]">            <title>Press arrow up or arrow down Activity</title>
            <Nav />
            <div className="flex flex-col items-center justify-center">
                <h1 className="text-2xl font-bold">Press arrow up or arrow down Buttons</h1>
                {/* image path */}
                <img src="/images/ArrowUpArrowDown-key-press.gif" alt="Press arrows" className="mt-4" />
                {showMarks && (
                    <p className="mt-4 text-lg">
                        ...
                        {correctPress ? (
                            ...
                            <span className="text-red-500">✗ You pressed {keyPressed}. Please press arrow up or arrow down buttons.</span>
                        )}
                    </p>
                )}
