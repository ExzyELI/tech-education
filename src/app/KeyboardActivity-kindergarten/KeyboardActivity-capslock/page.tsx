...
const CapslockKeyPressPage = () => {
    ...
    useEffect(() => {
        const handleKeyDown = async (e: KeyboardEvent) => {
            ...
            setCorrectPress(e.code === "CapsLock");
            setShowMarks(true);

            //if the correct key is pressed, save the activity data to the database
            if (e.code === "CapsLock" && !databaseRegistered) {
    ...
    const goToNextTask = () => {
        router.push("/KeyboardActivity-kindergarten/KeyboardActivity-shifts");
    };

    return (
        <main className="flex min-h-screen flex-col justify-between bg-[#FAF9F6] font-serif leading-normal tracking-normal text-[#132241]">            <title>Press Capslock Activity</title>
            <Nav />
