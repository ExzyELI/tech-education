...
const tabKeyPressPage = () => {
    ...
    const goToNextTask = () => {
        router.push("/KeyboardActivity-kindergarten/KeyboardActivity-capslock");
    };

    return (
        <main className="flex min-h-screen flex-col justify-between bg-[#FAF9F6] font-serif leading-normal tracking-normal text-[#132241]">            <title>Press tab Activity</title>
            <Nav />
            <div className="flex flex-col items-center justify-center">
                <h1 className="text-2xl font-bold">Press the tab Button</h1>
                {/* image path */}
                <img src="/images/tab-key-press.gif" alt="Press tab" className="mt-4" />
                {showMarks && (
                    <p className="mt-4 text-lg">
                        {/* display if right key is pressed */}
                        {correctPress ? (
                            <span className="text-green-500">✓ Correct! You pressed the tab button.</span>
                        ) : (
                            //display any other key pressed */}
                            <span className="text-red-500">✗ You pressed {keyPressed}. Please press the tab button.</span>
                        )}
                    </p>
                )}
...
export default tabKeyPressPage;
