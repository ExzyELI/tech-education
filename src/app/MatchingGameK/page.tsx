  ...
  const handleSave = async () => {
    ...
    if (user) {
      ...
      if (userDocSnap.exists()) {
        ...
        const currentAttempts = userData?.matchingK_attempts || 0; // 0 if no attempts found
        // update attempts locally
        setAttempts(currentAttempts + 1);
        // update attempts to firestore
        await setDoc(
          userDocRef,
          { matchingK_attempts: increment(1) },
          { merge: true },
        );
        // save activity data in firestore
        await addDoc(collection(firestore, `users/${user.uid}/activities`), {
          activityName: "Matching Activity K",
          score: finalScore,
          matchingK_attempts: currentAttempts + 1,
          timestamp: new Date(),
            ...
            {gameStarted && (
              <div>
                ...
                <div className="container mt-8 border-4 border-dashed border-[#5c93ff] bg-[#5ab2ff] px-3 py-2">
                  <section className="grid grid-cols-3 justify-items-center gap-3">
                    {cards.map((card, index) => (
                      <button
                        key={index}
                        className={`memory-card flex h-40 w-40 items-center justify-center rounded-lg bg-[#ff5a5f] text-white shadow-lg hover:bg-[#ff4146] focus:outline-none focus:ring focus:ring-black active:bg-[#ff4146] ${flippedCards.includes(index) || matchedCards.includes(index) ? "flipped" : ""}`}
                ...
                <div className="mt-4 flex justify-center">
                  <button
                    className="z-10 rounded bg-red-500 px-4 py-2 font-bold text-white shadow-md hover:bg-red-600"
                    onClick={resetGame}
                  >
                    Reset Game{" "}
                  </button>
