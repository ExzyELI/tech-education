...
const KeyVKeyPressPage = () => {
    ...
    useEffect(() => {
        const handleKeyDown = async (e: KeyboardEvent) => {
            ...
            //check if the KeyV button is pressed
            setCorrectPress(e.code === "KeyV");
            setShowMarks(true);

            //if the correct key is pressed, save the activity data to the database
            if (e.code === "KeyV" && !databaseRegistered) {
    ...
    const goToNextTask = () => {
        router.push("/KeyboardActivity-firstGrade/KeyboardActivity-W");
    };
    ...
    return (
        ...
        <title>Press Key V Activity</title>
            <Nav />
            <div className="flex flex-col items-center justify-center">
                {/* title and image path */}
                <h1 className="text-2xl font-bold">Press the Key V Button</h1>
                <img src="/images/KeyV-key-press.gif" alt="Press KeyV" className="mt-4" />
                {showMarks && (
                    <p className="mt-4 text-lg">
                        {/* show the user what key was pressed  */}
                        {correctPress ? (
                            <span className="text-green-500">✓ Correct! You pressed the Key V button.</span>
                        ) : (
                            <span className="text-red-500">✗ You pressed {keyPressed}. Please press the Key V button.</span>
                        )}
                    </p>
                )}
...
export default KeyVKeyPressPage;
