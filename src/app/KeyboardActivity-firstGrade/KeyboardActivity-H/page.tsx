...
const KeyHKeyPressPage = () => {
    ...
    useEffect(() => {
        const handleKeyDown = async (e: KeyboardEvent) => {
            ...
            //check if the KeyH button is pressed
            setCorrectPress(e.code === "KeyH");
            setShowMarks(true);

            //if the correct key is pressed, save the activity data to the database
            if (e.code === "KeyH" && !databaseRegistered) {
    ...
    const goToNextTask = () => {
        router.push("/KeyboardActivity-firstGrade/KeyboardActivity-I");
    };
    ...
    return (
        ...
        <title>Press Key H Activity</title>
            <Nav />
            <div className="flex flex-col items-center justify-center">
                {/* title and image path */}
                <h1 className="text-2xl font-bold">Press the Key H Button</h1>
                <img src="/images/KeyH-key-press.gif" alt="Press KeyH" className="mt-4" />
                {showMarks && (
                    <p className="mt-4 text-lg">
                        {/* show the user what key was pressed  */}
                        {correctPress ? (
                            <span className="text-green-500">✓ Correct! You pressed the Key H button.</span>
                        ) : (
                            <span className="text-red-500">✗ You pressed {keyPressed}. Please press the Key H button.</span>
                        )}
                    </p>
                )}
...
export default KeyHKeyPressPage;
