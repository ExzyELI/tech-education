...
const KeyOKeyPressPage = () => {
    ...
    useEffect(() => {
        const handleKeyDown = async (e: KeyboardEvent) => {
            ...
            //check if the KeyO button is pressed
            setCorrectPress(e.code === "KeyO");
            setShowMarks(true);

            //if the correct key is pressed, save the activity data to the database
            if (e.code === "KeyO" && !databaseRegistered) {
    ...
    const goToNextTask = () => {
        router.push("/KeyboardActivity-firstGrade/KeyboardActivity-P");
    };
    ...
    return (
        ...
        <title>Press Key O Activity</title>
            <Nav />
            <div className="flex flex-col items-center justify-center">
                {/* title and image path */}
                <h1 className="text-2xl font-bold">Press the Key O Button</h1>
                <img src="/images/KeyO-key-press.gif" alt="Press KeyO" className="mt-4" />
                {showMarks && (
                    <p className="mt-4 text-lg">
                        {/* show the user what key was pressed  */}
                        {correctPress ? (
                            <span className="text-green-500">✓ Correct! You pressed the Key O button.</span>
                        ) : (
                            <span className="text-red-500">✗ You pressed {keyPressed}. Please press the Key O button.</span>
                        )}
                    </p>
                )}
...
export default KeyOKeyPressPage;
