...
const KeyTKeyPressPage = () => {
    ...
    useEffect(() => {
        const handleKeyDown = async (e: KeyboardEvent) => {
            ...
            //check if the KeyT button is pressed
            setCorrectPress(e.code === "KeyT");
            setShowMarks(true);

            //if the correct key is pressed, save the activity data to the database
            if (e.code === "KeyT" && !databaseRegistered) {
    ...
    const goToNextTask = () => {
        router.push("/KeyboardActivity-firstGrade/KeyboardActivity-U");
    };
    ...
    return (
        ...
        <title>Press Key T Activity</title>
            <Nav />
            <div className="flex flex-col items-center justify-center">
                {/* title and image path */}
                <h1 className="text-2xl font-bold">Press the Key T Button</h1>
                <img src="/images/KeyT-key-press.gif" alt="Press KeyT" className="mt-4" />
                {showMarks && (
                    <p className="mt-4 text-lg">
                        {/* show the user what key was pressed  */}
                        {correctPress ? (
                            <span className="text-green-500">✓ Correct! You pressed the Key T button.</span>
                        ) : (
                            <span className="text-red-500">✗ You pressed {keyPressed}. Please press the Key T button.</span>
                        )}
                    </p>
                )}
...
export default KeyTKeyPressPage;
