...
const KeySKeyPressPage = () => {
    ...
    useEffect(() => {
        const handleKeyDown = async (e: KeyboardEvent) => {
            ...
            //check if the KeyS button is pressed
            setCorrectPress(e.code === "KeyS");
            setShowMarks(true);

            //if the correct key is pressed, save the activity data to the database
            if (e.code === "KeyS" && !databaseRegistered) {
    ...
    const goToNextTask = () => {
        router.push("/KeyboardActivity-firstGrade/KeyboardActivity-T");
    };
    ...
    return (
        ...
        <title>Press Key S Activity</title>
            <Nav />
            <div className="flex flex-col items-center justify-center">
                {/* title and image path */}
                <h1 className="text-2xl font-bold">Press the Key S Button</h1>
                <img src="/images/KeyS-key-press.gif" alt="Press KeyS" className="mt-4" />
                {showMarks && (
                    <p className="mt-4 text-lg">
                        {/* show the user what key was pressed  */}
                        {correctPress ? (
                            <span className="text-green-500">✓ Correct! You pressed the Key S button.</span>
                        ) : (
                            <span className="text-red-500">✗ You pressed {keyPressed}. Please press the Key S button.</span>
                        )}
                    </p>
                )}
...
export default KeySKeyPressPage;
